...
### Transaction-Grouped Account Updates (Geyser)

> **NOTE:** This feature is on the `add_tx_with_accounts_geyser` branch.
...
**Key files:**
- `geyser-plugin-interface/src/geyser_plugin_interface.rs` - `ReplicaTransactionAccountsInfo`, new trait methods
- `accounts-db/src/accounts_update_notifier_interface.rs` - `notify_transaction_accounts()` interface
- `geyser-plugin-manager/src/accounts_update_notifier.rs` - Implementation
- `runtime/src/bank.rs` - `notify_transaction_accounts_to_plugins()` in commit flow

**Gating:** Off by default. Enable with `--enable-transaction-accounts-notify`
(`ValidatorConfig::enable_transaction_accounts_notify`). Delivery is synchronous
on the commit path.
